...
const char *websocket_server = "192.168.4.10";
...
String currentModeCode; // Will store the current mode value
...
void establishConnectionWithCATserver()
{
    Serial.println("Initializing WebSocket...");
    printOnTFT("Connecting to CAT server...", TFT_YELLOW, TFT_BLACK);

    webSocket.begin(websocket_server, websocket_port);
    webSocket.onEvent(webSocketEvent);

    unsigned long startAttemptTime = millis(); // Record the start time
    unsigned long connectionTime = 0;          // Variable to store connection time

    while (!webSocket.isConnected() && millis() - startAttemptTime < 10000)
    {
        webSocket.loop();
        delay(200);
    ...
    {
        connectionTime = millis() - startAttemptTime; // Calculate connection time
        Serial.printf("WebSocket connected successfully in %lu ms!\n", connectionTime);
        connectedToSocket = true;

        // Display connection time on TFT
        char message[50];
        snprintf(message, sizeof(message), "Connected to CAT server: %lu ms", connectionTime);
        printOnTFT(message, TFT_GREEN, TFT_BLACK);
        printOnTFT("", TFT_GREEN, TFT_BLACK);
        printOnTFT("     We Are Ready To Go !!!!", TFT_GREEN, TFT_BLACK);
    }
    else
    {
        Serial.println("WebSocket connection failed. Will retry in loop.");
        printOnTFT("WebSocket connection failed", TFT_RED, TFT_BLACK);
        printOnTFT("", TFT_RED, TFT_BLACK);
        printOnTFT("Will retry", TFT_RED, TFT_BLACK);
    }
}
...
void webSocketEvent(WStype_t type, uint8_t *payload, size_t length)
{
    ...
    case WStype_CONNECTED:
        Serial.println("Event: Connected to WebSocket server.");
        connectedToSocket = true;
        webSocket.sendTXT("Hello, ESP32 MLA MASTER has connected!!");
        break;

    case WStype_DISCONNECTED:
        Serial.println("Event: Disconnected from WebSocket server.");
        connectedToSocket = false;
        break;

    case WStype_TEXT:
    {
        Serial.printf("Event: Received message: %s\n", payload);

        // Preprocess payload: Replace single quotes with double quotes and None with null
        String processedPayload = String((char *)payload);
        processedPayload.replace("'", "\"");
        processedPayload.replace("None", "null");

        // JSON Parsing
        DynamicJsonDocument doc(512); // Explicitly scoped here
        DeserializationError error = deserializeJson(doc, processedPayload.c_str());
        ...
        // Extract and assign values using doc["key"].is<T>() for checks
        if (doc["Mode"].is<const char *>())
        {
            currentModeName = doc["Mode"].as<String>();
            currentModeCode = getModeCodeFromName(currentModeName);

            // Debugging outputs
            Serial.print("Mode Name: ");
            Serial.print(currentModeName);

            Serial.print(" (Mode Code (as String): ");
            Serial.print(currentModeCode);
            Serial.println(")");
        }

        if (doc["RFPower"].is<const char *>())
        {
            String powerStr = doc["RFPower"].as<String>();                     // "50W"
            setRFPower = powerStr.substring(0, powerStr.length() - 1).toInt(); // Remove 'W' and convert to integer
            Serial.printf("Parsed RFPower: %d\n", setRFPower);
        }

        if (doc["VFOFrequency"].is<const char *>())
        {
            CurrentVFOFrequency = doc["VFOFrequency"].as<unsigned long>();
        ...
        {
            String pttStr = doc["PTTStatus"].as<String>(); // "ON" or "OFF"
            PTTstatus = (pttStr == "ON");
            Serial.printf("Parsed PTTStatus: %s\n", PTTstatus ? "ON" : "OFF");
        }
        break;
    }

    default:
        Serial.printf("Event: Unhandled WebSocket event type: %d\n", type);
        break;
    }
}
...
void sendCommandToCATserver(String command)
{
    if (connectedToSocket)
    {
        ...
        Serial.printf("Sent command to Python: %s\n", command.c_str());
    }
    else
    {
        Serial.println("WebSocket is not connected. Cannot send command.");
    }
}

// Function to set TX status
...
void setPower(int power)
{
    ...
    {
        Serial.println("Invalid power value. Must be between 5 and 100.");
        return;
    }
...
{
    String validModes[] = {"USB", "LSB", "FM", "AM", "CW-U", "CW-L", "DATA-U", "DATA-L"};
    bool isValid = false;
...
void setFrequ(long frequ)
{
    if (frequ < 0 || frequ > 30000000)
    { // 0 to 30 MHz
        Serial.println("Invalid frequency. Must be between 0 and 30 MHz.");
        return;
    }
...
void setup()
{
    ...
    establishConnectionWithCATserver();
    delay(2000);
    // Clear display
    tft.fillScreen(TFT_BLACK);
