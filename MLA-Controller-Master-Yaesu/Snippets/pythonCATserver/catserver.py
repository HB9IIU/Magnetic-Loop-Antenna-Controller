...
def get_wifi_ip(interface):
    ...
    try:
         # Wi-Fi interface name on Raspberry Pi
...
def swr_from_swr_cat_value(x):
    """
    Calculate the SWR value based on the SWRcatValue using a 5th-order polynomial fit.

    Parameters:
        x (float): The SWRcatValue.

    Returns:
        float: The corresponding SWR value rounded to one decimal place.
    """
    result = (
        5.84151360e-10 * x ** 5
        - 1.51007177e-07 * x ** 4
...
# Example usage:
swr_value = swr_from_swr_cat_value(63)  # Replace 63 with your SWRcatValue
print(f"SWR Value: {swr_value}")
