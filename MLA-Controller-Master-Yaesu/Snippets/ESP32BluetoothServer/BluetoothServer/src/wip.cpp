...
const unsigned long updateInterval = 25; // Update every 100 ms
...
void parseCATResponses() {
  ...
  bool stateChanged = false;          // Track if any state changed

  String response = sendCATCommand(CAT_COMMANDS[currentCommandIndex]);

  if (currentCommandIndex == 0) { // VFO Frequency and Mode
    if (response.length() >= 22 && response.startsWith("IF") && response.endsWith(";")) {
      unsigned long newFrequency = response.substring(5, 14).toInt();
      char modeCode = response[21];
      String newMode = modeCodeToName(modeCode);

      if (newFrequency != VFOFrequency || newMode != Mode) {
        VFOFrequency = newFrequency;
        Mode = newMode;
        stateChanged = true;
      }
    }
  } else if (currentCommandIndex == 1) { // PTT Status
    if (response.startsWith("TX") && response.length() >= 3) {
      int status = response[2] - '0';
      String newPTTStatus = (status == 1 || status == 2) ? "ON" : "OFF";

      if (newPTTStatus != PTTStatus) {
        PTTStatus = newPTTStatus;
        stateChanged = true;
      }
    }
  } else if (currentCommandIndex == 2) { // RF Power
    if (response.startsWith("PC") && response.length() >= 5 && response.endsWith(";")) {
      int newRFPower = response.substring(3, 5).toInt();

      if (newRFPower != RFPower) {
        RFPower = newRFPower;
        stateChanged = true;
      }
    }
  } else if (currentCommandIndex == 3 && PTTStatus == "ON") { // SWR (if PTT is ON)
    if (response.startsWith("RM") && response.length() >= 6 && response.endsWith(";")) {
      int swrCatValue = response.substring(3, 6).toInt();
      float newSWR = calculateSWR(swrCatValue);
  ...
  // If any state changed, flicker the red LED
  if (stateChanged) {
    digitalWrite(RED_LED_PIN, HIGH);
    redLEDTurnOffTime = millis() + 100; // Turn off after 100ms
    redLEDOn = true;
  }
}
...
void setup()
{
  ...
  // Turn on the red LED to indicate power ON
  digitalWrite(RED_LED_PIN, HIGH);
...
void loop() {
  ...
  // Check if the red LED timer has expired
  if (redLEDOn && millis() > redLEDTurnOffTime) {
  ...
  if (currentTime - lastUpdateTime >= updateInterval) {
    ...
    if (deviceConnected && sendAllVariables) {
      sendAllVariablesToClient();
    }

    // Send notifications for changed variables
    if (deviceConnected) {
      sendUpdatedVariables();
    }
  }
...
//just to see somethong on the console
if (VFOFrequency != prevVFOFrequency) {
    String message = "FREQ:" + String(VFOFrequency);
    pNotifyCharacteristic->setValue(message.c_str());
    pNotifyCharacteristic->notify();
