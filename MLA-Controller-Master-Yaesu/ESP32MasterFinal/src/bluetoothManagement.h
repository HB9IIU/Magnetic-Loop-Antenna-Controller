#ifndef BLUETOOTH_MANAGEMENT_H
#define BLUETOOTH_MANAGEMENT_H
...
// uncomment to run isolated
unsigned long CurrentVFOFrequency;
...
static void notifyCallback(BLERemoteCharacteristic *pBLERemoteCharacteristic, uint8_t *pData, size_t length, bool isNotify)
{
    ...
    if (prefix == "FREQ")
    {
        CurrentVFOFrequency = data.toInt();
        Serial.print("VFO Frequency: ");
        Serial.println(CurrentVFOFrequency);
    }
    else if (prefix == "MODE")
    {
        currentModeName = data;
        Serial.print("Mode: ");
    ...
    {
        setRFPower = data.toInt();
        Serial.print("RF Power: ");
        Serial.println(setRFPower);
    }
    else if (prefix == "SWR")
    {
        currentSWR = data.toFloat();
        Serial.print("SWR: ");
...
bool resetScan(unsigned long timeoutMs)
{
    ...
    // Run the scan and keep checking for timeout
    while (millis() - startTime < timeoutMs)
    {
        scanResults = pBLEScan->start(1, false); // Scan in small chunks (1 second)
        if (myDevice != nullptr)                 // If a matching device is found
        {
            Serial.println("Device found!");
            return true; // Exit and signal success
        }
    }

    Serial.println("Scan timed out. No matching device found.");
    return false; // Exit and signal failure
}

void setup_ForTestingIsolated()
{
    Serial.begin(115200);
    delay(2000);
...
void setTXon()
{
    String command = "SETPTT:ON";
    pWriteCharacteristic->writeValue(command.c_str());
    Serial.println("PTT command sent: " + command);
}
void setTXoff()
{
    String command = "SETPTT:OFF";
    pWriteCharacteristic->writeValue(command.c_str());
    Serial.println("PTT command sent: " + command);
}

void loop_ForTestingIsolated()
{
